...
Tái cấu trúc hoàn toàn từ 3328 dòng monolithic xuống < 150 dòng delegate.from tkinter import messagebox, ttk, colorchooser
...
from processors.color_processor import ColorProcessor    This class provides various image processing operations including:

from processors.geometric_processor import GeometricProcessor    - Color space conversions (RGB, HSV, Grayscale)
...
from processors.intensity_processor import IntensityProcessor    - Morphological operations (erode, dilate, open, close)

from processors.advanced_processor import AdvancedProcessor    - Intensity transformations (log, power/gamma)

from processors.drawing_processor import DrawingProcessor    - Advanced operations (registration, stitching)

    - Drawing utilities (line, rectangle, circle, text)

    

class FunctionsProcessing:    All operations return a tuple of (processed_image, code_string) where
    ...
    def __init__(self, Image=None, ImageTk=None):        
        ...
        Initialize FunctionsProcessing with PIL dependencies.            pil_image_module: PIL.Image module
        ...
        Args:        """
            ...
            ImageTk: PIL.ImageTk module (for Tkinter display)        self.ImageTk = pil_image_tk_module

        """

        # Store PIL dependencies (injected from GUI)    # ========================================================================

        self.Image = Image    # COLOR SPACE CONVERSIONS

        self.ImageTk = ImageTk    # ========================================================================

            

        # Initialize all specialized processors with PIL dependencies    def cvt_Negative(self, image):

        self.color_proc = ColorProcessor(Image, ImageTk)        """Convert to Negative image color space."""

        self.geometric_proc = GeometricProcessor(Image, ImageTk)        return cv2.bitwise_not(image), "cv2.bitwise_not(image)\n"

        self.filter_proc = FilterProcessor(Image, ImageTk)

        self.segmentation_proc = SegmentationProcessor(Image, ImageTk)    def cvt_HSV(self, image):

        self.morphology_proc = MorphologyProcessor(Image, ImageTk)        """Convert BGR to HSV color space."""

        self.intensity_proc = IntensityProcessor(Image, ImageTk)        return cv2.cvtColor(image, cv2.COLOR_BGR2HSV), "cv2.cvtColor(image, cv2.COLOR_BGR2HSV)\n"
    ...
    # COLOR CONVERSIONS - Delegate to ColorProcessor    
    ...
    def cvt_Negative(self, image):    # ========================================================================
        ...
        return self.color_proc.cvt_Negative(image)    def equalized_image(self, image):

            """Apply histogram equalization to enhance contrast."""

    def cvt_HSV(self, image):        # Check if image is grayscale

        """Convert BGR to HSV color space."""        if len(image.shape) > 2 and image.shape[2] > 1:

        return self.color_proc.cvt_HSV(image)            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

                equalized = cv2.equalizeHist(gray)

    def cvt_GRAY(self, image):            return equalized, "gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)\nequalized = cv2.equalizeHist(gray)\n"

        """Convert BGR to Grayscale."""        else:

        return self.color_proc.cvt_GRAY(image)            try:

                    return cv2.equalizeHist(image), "equalized = cv2.equalizeHist(image)\n"

    # ========================================================================            except Exception:

    # GEOMETRIC TRANSFORMATIONS - Delegate to GeometricProcessor                messagebox.showerror("Error", "Failed to equalize histogram. Image format not supported.")
    ...
    def rotate_image(self, image):    # ========================================================================
        ...
        """Rotate image by 90 degrees clockwise."""    # GEOMETRIC TRANSFORMATIONS

        return self.geometric_proc.rotate_image(image)    # ========================================================================

        

    def resize_image(self, image):    def rotate_image(self, image):

        """Resize image with custom dimensions."""        """Rotate image by 90 degrees clockwise."""

        return self.geometric_proc.resize_image(image)        return cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE), "cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE)\n"

    

    def flip_Horizontal_image(self, image):    def resize_image(self, image):

        """Flip image horizontally."""        """Resize image with custom dimensions or percentage."""

        return self.geometric_proc.flip_Horizontal_image(image)        # Create dialog for custom resize
    ...
    def flip_Vertical_image(self, image):        dialog = tk.Toplevel()
        ...
        return self.geometric_proc.flip_Vertical_image(image)        dialog.geometry("400x300")

            dialog.resizable(False, False)

    def rotationMatrix2d(self, image):        dialog.grab_set()  # Make dialog modal
    ...
    def move_image(self, image):        
        ...
        """Translate/move image in X and Y directions."""        tk.Label(dialog, text="Original dimensions:", font=("Arial", 12)).pack(pady=(10, 5))

        return self.geometric_proc.move_image(image)        tk.Label(dialog, text=f"Width: {w}px, Height: {h}px", font=("Arial", 11)).pack(pady=(0, 10))

            

    def perspectiveTransform(self, image):        # Create frame for resize options

        """Apply perspective transformation."""        options_frame = ttk.Frame(dialog)

        return self.geometric_proc.perspectiveTransform(image)        options_frame.pack(fill=tk.X, padx=20, pady=10)
