...
from .processing import FunctionsProcessing # <-- Thay đổi import
import os
...
class GUI:
    ...
    def setUpGUI(self):
        ...
        self.create_function_category(left_panel, "Morphological Ops", [
            ("Erode / Dilate / Open / Close", "Morphology"), # <-- NÚT MỚI
        ])
        
        # Sắp xếp lại các nhóm bên phải
        self.create_function_category(right_panel, "Segmentation", [
            ("Global Threshold", "Threshold"),
            ("Adaptive Threshold", "Adaptive Threshold"), # <-- NÚT MỚI
            ("Canny Edge", "Canny"),
        ])

        self.create_function_category(right_panel, "Filters & Enhancement", [
            ("Histogram Equalize", "Equalized"),
            ("Gaussian Blur", "GaussianBlur Dialog"),   # <-- THAY ĐỔI
            ("Median Blur", "MedianBlur Dialog"),     # <-- THAY ĐỔI
        ])
        ...
        # Code display
        ttk.Label(bottom_panel, text="OpenCV Code:", font=("Arial", 12, "bold")).pack(anchor=W, pady=(10, 5))
        code_frame = ttk.Frame(bottom_panel)
        code_frame.pack(fill=BOTH, expand=True, padx=5, pady=5)
        
        self.code_display = Text(code_frame, font=("Consolas", 12), width=80, height=100, bg="#f8f8f8", wrap=NONE)
        self.code_display.pack(fill=BOTH, expand=True, padx=5, pady=5)
        
        # Add scrollbars to code display
        code_y_scrollbar = ttk.Scrollbar(self.code_display, orient="vertical", command=self.code_display.yview)
        code_y_scrollbar.pack(side=RIGHT, fill=Y)
        
        code_x_scrollbar = ttk.Scrollbar(code_frame, orient="horizontal", command=self.code_display.xview)
        code_x_scrollbar.pack(side=BOTTOM, fill=X)
        
        self.code_display.config(yscrollcommand=code_y_scrollbar.set, xscrollcommand=code_x_scrollbar.set)
        
        # Configure code text trace
        self.set_code_text.trace("w", lambda *args: self.update_code_display())
    ...
    def create_function_category(self, parent, title, functions):
        ...
        for label, transformation in functions:
            btn = ttk.Button(frame, text=label, width=25,  # <-- Tăng chiều rộng nút
    ...
    def apply_transformation(self, transformation):
        ...
        func_map = {
            ...
            "GaussianBlur Dialog" : self.fp.gaussian_blur_dialog, # <-- NÂNG CẤP
            "MedianBlur Dialog"   : self.fp.median_blur_dialog,   # <-- NÂNG CẤP
            "Morphology"          : self.fp.morph_operations_dialog, # <-- MỚI
            "Adaptive Threshold"  : self.fp.adaptive_threshold_dialog, # <-- MỚI
        }
        # ===========================
        
        # Bỏ các hàm hard-coded cũ
