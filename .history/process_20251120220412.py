...
Image Processing Module - REFACTORED VERSION

Image Processing Module - REFACTORED VERSIONimport tkinter as tk

Tái cấu trúc hoàn toàn từ 3328 dòng monolithic xuống 200 dòng delegate.

Code thật nằm trong processors/ và process_monolithic_backup.pyTái cấu trúc hoàn toàn từ 3328 dòng monolithic xuống < 150 dòng delegate.from tkinter import messagebox, ttk, colorchooser



Architecture: import numpy as np

- process.py (file này): Lightweight wrapper class - chỉ delegate

- processors/*.py: Import và wrap code từ process_monolithic_backup.pyArchitecture: 

- process_monolithic_backup.py: File gốc 3328 dòng (backup)

- process.py: Lightweight wrapper/facade class

Benefit: Code gọn gàng, dễ đọc, dễ maintain. Nhưng vẫn dùng TOÀN BỘ logic gốc!

"""- processors/*: Specialized processing modulesclass FunctionsProcessing:
...
from processors.geometric_processor import GeometricProcessor    Image Processing Functions Handler

from processors.filter_processor import FilterProcessor

from processors.segmentation_processor import SegmentationProcessor# Import all specialized processors    

from processors.morphology_processor import MorphologyProcessor

from processors.intensity_processor import IntensityProcessorfrom processors.color_processor import ColorProcessor    This class provides various image processing operations including:

from processors.advanced_processor import AdvancedProcessor

from processors.drawing_processor import DrawingProcessorfrom processors.geometric_processor import GeometricProcessor    - Color space conversions (RGB, HSV, Grayscale)



from processors.filter_processor import FilterProcessor    - Geometric transformations (resize, flip, rotate, move, perspective)

class FunctionsProcessing:

    """from processors.segmentation_processor import SegmentationProcessor    - Filters and enhancement (blur, contrast, histogram equalization)

    Main image processing class - delegates to specialized processors.

    from processors.morphology_processor import MorphologyProcessor    - Segmentation and edge detection (thresholding, Canny)

    Đây là class chính mà GUI gọi. Nó delegate tất cả operations

    đến các processors chuyên biệt, giữ code gọn gàng và dễ maintain.from processors.intensity_processor import IntensityProcessor    - Morphological operations (erode, dilate, open, close)

    """

    from processors.advanced_processor import AdvancedProcessor    - Intensity transformations (log, power/gamma)

    def __init__(self, Image=None, ImageTk=None):

        """Initialize FunctionsProcessing with PIL dependencies."""from processors.drawing_processor import DrawingProcessor    - Advanced operations (registration, stitching)

        self.Image = Image

        self.ImageTk = ImageTk    - Drawing utilities (line, rectangle, circle, text)
        ...
        self.geometric_proc = GeometricProcessor(Image, ImageTk)class FunctionsProcessing:    All operations return a tuple of (processed_image, code_string) where

        self.filter_proc = FilterProcessor(Image, ImageTk)

        self.segmentation_proc = SegmentationProcessor(Image, ImageTk)    """    code_string contains the OpenCV code to reproduce the operation.

        self.morphology_proc = MorphologyProcessor(Image, ImageTk)

        self.intensity_proc = IntensityProcessor(Image, ImageTk)    Main image processing class - delegates to specialized processors.    """
    ...
    # ========================================================================    Đây là class chính mà GUI gọi. Nó delegate tất cả operations    # === INITIALIZATION ===

    # COLOR CONVERSIONS

    # ========================================================================    đến các processors chuyên biệt, giữ code gọn gàng và dễ maintain.    def __init__(self, pil_image_module, pil_image_tk_module):
    ...
    def cvt_Negative(self, image):    """        """
        ...
        return self.color_proc.cvt_Negative(image)            Initialize with PIL modules via dependency injection.

    

    def cvt_HSV(self, image):    def __init__(self, Image=None, ImageTk=None):        

        """Convert BGR to HSV color space."""

        return self.color_proc.cvt_HSV(image)        """        Args:

    

    def cvt_GRAY(self, image):        Initialize FunctionsProcessing with PIL dependencies.            pil_image_module: PIL.Image module

        """Convert BGR to Grayscale."""

        return self.color_proc.cvt_GRAY(image)                    pil_image_tk_module: PIL.ImageTk module
    ...
    # ========================================================================            Image: PIL.Image module (for image conversion)        self.Image = pil_image_module

    

    def rotate_image(self, image):            ImageTk: PIL.ImageTk module (for Tkinter display)        self.ImageTk = pil_image_tk_module
    ...
    def resize_image(self, image):        # Store PIL dependencies (injected from GUI)    # ========================================================================

        """Resize image with custom dimensions."""

        return self.geometric_proc.resize_image(image)        self.Image = Image    # COLOR SPACE CONVERSIONS

    

    def flip_Horizontal_image(self, image):        self.ImageTk = ImageTk    # ========================================================================
    ...
    def flip_Vertical_image(self, image):        # Initialize all specialized processors with PIL dependencies    def cvt_Negative(self, image):

        """Flip image vertically."""

        return self.geometric_proc.flip_Vertical_image(image)        self.color_proc = ColorProcessor(Image, ImageTk)        """Convert to Negative image color space."""

    

    def rotationMatrix2d(self, image):        self.geometric_proc = GeometricProcessor(Image, ImageTk)        return cv2.bitwise_not(image), "cv2.bitwise_not(image)\n"

        """Apply custom rotation with angle and scale."""

        return self.geometric_proc.rotationMatrix2d(image)        self.filter_proc = FilterProcessor(Image, ImageTk)

    

    def move_image(self, image):        self.segmentation_proc = SegmentationProcessor(Image, ImageTk)    def cvt_HSV(self, image):

        """Translate/move image in X and Y directions."""

        return self.geometric_proc.move_image(image)        self.morphology_proc = MorphologyProcessor(Image, ImageTk)        """Convert BGR to HSV color space."""

    

    def perspectiveTransform(self, image):        self.intensity_proc = IntensityProcessor(Image, ImageTk)        return cv2.cvtColor(image, cv2.COLOR_BGR2HSV), "cv2.cvtColor(image, cv2.COLOR_BGR2HSV)\n"

        """Apply perspective transformation."""

        return self.geometric_proc.perspectiveTransform(image)        self.advanced_proc = AdvancedProcessor(Image, ImageTk)

    

    # ========================================================================        self.drawing_proc = DrawingProcessor(Image, ImageTk)    def cvt_GRAY(self, image):
    ...
    def equalized_image(self, image):    # ========================================================================        return cv2.cvtColor(image, cv2.COLOR_BGR2GRAY), "cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)\n"

        """Apply histogram equalization."""

        return self.filter_proc.equalized_image(image)    # COLOR CONVERSIONS - Delegate to ColorProcessor    
    ...
    def gaussian_blur_dialog(self, image):    # ========================================================================    # ========================================================================
        ...
        return self.filter_proc.gaussian_blur_dialog(image)        # FILTERS & ENHANCEMENT

    

    def median_blur_dialog(self, image):    def cvt_Negative(self, image):    # ========================================================================

        """Apply median blur with dialog."""

        return self.filter_proc.median_blur_dialog(image)        """Convert to Negative image."""    

    

    def bilateral_filter_dialog(self, image):        return self.color_proc.cvt_Negative(image)    def equalized_image(self, image):

        """Apply bilateral filter with dialog."""

        return self.filter_proc.bilateral_filter_dialog(image)            """Apply histogram equalization to enhance contrast."""

    

    def canny_detection(self, image):    def cvt_HSV(self, image):        # Check if image is grayscale

        """Detect edges using Canny edge detector."""

        return self.filter_proc.canny_detection(image)        """Convert BGR to HSV color space."""        if len(image.shape) > 2 and image.shape[2] > 1:

    

    # ========================================================================        return self.color_proc.cvt_HSV(image)            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    def binary_threshold_dialog(self, image):    def cvt_GRAY(self, image):            return equalized, "gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)\nequalized = cv2.equalizeHist(gray)\n"

        """Apply binary threshold with dialog."""

        return self.segmentation_proc.binary_threshold_dialog(image)        """Convert BGR to Grayscale."""        else:

    

    def adaptive_threshold_dialog(self, image):        return self.color_proc.cvt_GRAY(image)            try:

        """Apply adaptive threshold with dialog."""

        return self.segmentation_proc.adaptive_threshold_dialog(image)                    return cv2.equalizeHist(image), "equalized = cv2.equalizeHist(image)\n"

    

    def otsu_threshold(self, image):    # ========================================================================            except Exception:

        """Apply Otsu's threshold."""

        return self.segmentation_proc.otsu_threshold(image)    # GEOMETRIC TRANSFORMATIONS - Delegate to GeometricProcessor                messagebox.showerror("Error", "Failed to equalize histogram. Image format not supported.")
    ...
    def morphology_dialog(self, image):    def rotate_image(self, image):    # ========================================================================

        """Apply morphological operations with dialog."""

        return self.morphology_proc.morph_operations_dialog(image)        """Rotate image by 90 degrees clockwise."""    # GEOMETRIC TRANSFORMATIONS
    ...
    def histogram_calculation(self, image):    def resize_image(self, image):    def rotate_image(self, image):

        """Calculate and display histogram."""

        return self.intensity_proc.histogram_calculation(image)        """Resize image with custom dimensions."""        """Rotate image by 90 degrees clockwise."""

    

    def contrast_enhancement_dialog(self, image):        return self.geometric_proc.resize_image(image)        return cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE), "cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE)\n"
    ...
    def log_transform_dialog(self, image):    def flip_Horizontal_image(self, image):    def resize_image(self, image):

        """Apply log transformation."""

        return self.intensity_proc.log_transform_dialog(image)        """Flip image horizontally."""        """Resize image with custom dimensions or percentage."""

    

    def power_transform_dialog(self, image):        return self.geometric_proc.flip_Horizontal_image(image)        # Create dialog for custom resize

        """Apply power-law (gamma) transformation."""

        return self.intensity_proc.power_transform_dialog(image)            result = None

    

    # ========================================================================    def flip_Vertical_image(self, image):        dialog = tk.Toplevel()

    # ADVANCED

    # ========================================================================        """Flip image vertically."""        dialog.title("Resize Image")

    

    def image_registration_dialog(self, image):        return self.geometric_proc.flip_Vertical_image(image)        dialog.geometry("400x300")

        """Register two images using feature matching."""

        return self.advanced_proc.image_registration_dialog(image)            dialog.resizable(False, False)

    

    def image_stitching_dialog(self, image):    def rotationMatrix2d(self, image):        dialog.grab_set()  # Make dialog modal

        """Stitch multiple images into panorama."""

        return self.advanced_proc.image_stitching_dialog(image)        """Apply custom rotation with angle and scale."""        

    

    # ========================================================================        return self.geometric_proc.rotationMatrix2d(image)        # Get original dimensions
    ...
    def draw_Line(self, image):    def move_image(self, image):        

        """Draw lines on image."""

        return self.drawing_proc.draw_Line(image)        """Translate/move image in X and Y directions."""        tk.Label(dialog, text="Original dimensions:", font=("Arial", 12)).pack(pady=(10, 5))

    

    def draw_Rectangle(self, image):        return self.geometric_proc.move_image(image)        tk.Label(dialog, text=f"Width: {w}px, Height: {h}px", font=("Arial", 11)).pack(pady=(0, 10))
    ...
    def draw_Circle(self, image):    def perspectiveTransform(self, image):        # Create frame for resize options

        """Draw circles on image."""

        return self.drawing_proc.draw_Circle(image)        """Apply perspective transformation."""        options_frame = ttk.Frame(dialog)

    

    def draw_text(self, image):        return self.geometric_proc.perspectiveTransform(image)        options_frame.pack(fill=tk.X, padx=20, pady=10)
    ...
    # FILTERS & ENHANCEMENT - Delegate to FilterProcessor        resize_mode = tk.StringVar(value="dimensions")

    # ========================================================================        maintain_aspect = tk.BooleanVar(value=True)

            width_var = tk.IntVar(value=w)

    def equalized_image(self, image):        height_var = tk.IntVar(value=h)

        """Apply histogram equalization."""        percent_var = tk.DoubleVar(value=100.0)

        return self.filter_proc.equalized_image(image)        

            # Functions to update fields

    def gaussian_blur_dialog(self, image):        def update_by_percent(*args):

        """Apply Gaussian blur with dialog."""            if resize_mode.get() == "percent":

        return self.filter_proc.gaussian_blur_dialog(image)                p = percent_var.get() / 100.0

                    width_var.set(int(w * p))

    def median_blur_dialog(self, image):                height_var.set(int(h * p))

        """Apply median blur with dialog."""        

        return self.filter_proc.median_blur_dialog(image)        def update_height(*args):

                if maintain_aspect.get() and resize_mode.get() == "dimensions":

    def bilateral_filter_dialog(self, image):                ratio = w / h

        """Apply bilateral filter with dialog."""                new_height = int(width_var.get() / ratio)

        return self.filter_proc.bilateral_filter_dialog(image)                height_var.set(new_height)

                    percent_var.set(round(width_var.get() / w * 100, 1))

    def canny_detection(self, image):        

        """Detect edges using Canny edge detector."""        def update_width(*args):

        return self.filter_proc.canny_detection(image)            if maintain_aspect.get() and resize_mode.get() == "dimensions":

                    ratio = w / h

    # ========================================================================                new_width = int(height_var.get() * ratio)

    # SEGMENTATION - Delegate to SegmentationProcessor                width_var.set(new_width)

    # ========================================================================                percent_var.set(round(height_var.get() / h * 100, 1))

            

    def binary_threshold_dialog(self, image):        def mode_changed(*args):

        """Apply binary threshold with dialog."""            if resize_mode.get() == "percent":

        return self.segmentation_proc.binary_threshold_dialog(image)                percent_frame.pack(fill=tk.X, pady=10)

                    dimensions_frame.pack_forget()

    def adaptive_threshold_dialog(self, image):                update_by_percent()

        """Apply adaptive threshold with dialog."""            else:

        return self.segmentation_proc.adaptive_threshold_dialog(image)                dimensions_frame.pack(fill=tk.X, pady=10)

                    percent_frame.pack_forget()

    def otsu_threshold(self, image):        

        """Apply Otsu's threshold."""        # Create resize options

        return self.segmentation_proc.otsu_threshold(image)        modes_frame = ttk.Frame(options_frame)

            modes_frame.pack(fill=tk.X, pady=5)

    # ========================================================================        

    # MORPHOLOGY - Delegate to MorphologyProcessor        ttk.Radiobutton(modes_frame, text="Dimensions", variable=resize_mode, 
    ...
    def morphology_dialog(self, image):                      value="percent", command=mode_changed).pack(side=tk.LEFT, padx=5)

        """Apply morphological operations with dialog."""        

        return self.morphology_proc.morphology_dialog(image)        # Dimensions frame

            dimensions_frame = ttk.Frame(options_frame)

    # ========================================================================        dimensions_frame.pack(fill=tk.X, pady=10)

    # INTENSITY TRANSFORMATIONS - Delegate to IntensityProcessor        

    # ========================================================================        width_frame = ttk.Frame(dimensions_frame)

            width_frame.pack(fill=tk.X, pady=5)

    def histogram_calculation(self, image):        ttk.Label(width_frame, text="Width:").pack(side=tk.LEFT, padx=5)

        """Calculate and display histogram."""        width_entry = ttk.Entry(width_frame, textvariable=width_var, width=10)

        return self.intensity_proc.histogram_calculation(image)        width_entry.pack(side=tk.LEFT, padx=5)

            ttk.Label(width_frame, text="px").pack(side=tk.LEFT)

    def contrast_enhancement_dialog(self, image):        

        """Enhance image contrast."""        height_frame = ttk.Frame(dimensions_frame)

        return self.intensity_proc.contrast_enhancement_dialog(image)        height_frame.pack(fill=tk.X, pady=5)

            ttk.Label(height_frame, text="Height:").pack(side=tk.LEFT, padx=5)

    def log_transform_dialog(self, image):        height_entry = ttk.Entry(height_frame, textvariable=height_var, width=10)

        """Apply log transformation."""        height_entry.pack(side=tk.LEFT, padx=5)

        return self.intensity_proc.log_transform_dialog(image)        ttk.Label(height_frame, text="px").pack(side=tk.LEFT)

            

    def power_transform_dialog(self, image):        # Percentage frame

        """Apply power-law (gamma) transformation."""        percent_frame = ttk.Frame(options_frame)
    ...
    # ADVANCED - Delegate to AdvancedProcessor        percent_scale.pack(pady=5, fill=tk.X)

    # ========================================================================        

            percent_value_frame = ttk.Frame(percent_frame)

    def image_registration_dialog(self, image):        percent_value_frame.pack(fill=tk.X, pady=5)

        """Register two images using feature matching."""        ttk.Label(percent_value_frame, text="Scale:").pack(side=tk.LEFT, padx=5)

        return self.advanced_proc.image_registration_dialog(image)        percent_entry = ttk.Entry(percent_value_frame, textvariable=percent_var, width=10)

            percent_entry.pack(side=tk.LEFT, padx=5)

    def image_stitching_dialog(self, image):        ttk.Label(percent_value_frame, text="%").pack(side=tk.LEFT)

        """Stitch multiple images into panorama."""        

        return self.advanced_proc.image_stitching_dialog(image)        # Aspect ratio checkbox

            ttk.Checkbutton(options_frame, text="Maintain aspect ratio", variable=maintain_aspect).pack(pady=10)

    # ========================================================================        

    # DRAWING - Delegate to DrawingProcessor        # Register callbacks

    # ========================================================================        width_var.trace("w", update_height)

            height_var.trace("w", update_width)

    def draw_Line(self, image):        percent_var.trace("w", update_by_percent)

        """Draw lines on image."""        

        return self.drawing_proc.draw_Line(image)        # Control buttons

            buttons_frame = ttk.Frame(dialog)

    def draw_Rectangle(self, image):        buttons_frame.pack(fill=tk.X, padx=20, pady=20)

        """Draw rectangles on image."""        

        return self.drawing_proc.draw_Rectangle(image)        def resize_ok():

                nonlocal result

    def draw_Circle(self, image):            try:

        """Draw circles on image."""                new_width = width_var.get()

        return self.drawing_proc.draw_Circle(image)                new_height = height_var.get()

                    

    def draw_text(self, image):                if new_width <= 0 or new_height <= 0:

        """Add text to image."""                    messagebox.showerror("Error", "Width and height must be positive values")

        return self.drawing_proc.draw_text(image)                    return
