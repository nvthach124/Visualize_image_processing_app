...
class MorphologyProcessor(BaseProcessor):
    ...
    def morph_operations_dialog(self, image):
        ...
        dialog, main_frame, canvas, controls, buttons, result_ref = self._create_basic_preview_dialog(
            "Morphological Operations", "700x600"
        )
        ...
        def apply_morph():
            ...
            code = f"# Morphological operation: {op_name}\n"
            code += f"kernel = np.ones(({k}, {k}), np.uint8)\n"
            code += f"result = cv2.morphologyEx(image, {op_code}, kernel, iterations={iters})\n"
            
            result_ref[0] = (morphed, code)
            dialog.destroy()
