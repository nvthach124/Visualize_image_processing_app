...
class FilterProcessor(BaseProcessor):
    ...
    def _create_basic_preview_dialog(self, title, geometry="700x500"):
        ...
        dialog = tk.Toplevel()
        dialog.title(title)
        self.center_window(dialog, geometry)
        dialog.grab_set()
    ...
    def canny_detection(self, image):
        ...
        t1_label = ttk.Label(threshold1_frame, text="100", width=3)
        t1_label.pack(side=tk.LEFT, padx=5)
        ...
        t2_label = ttk.Label(threshold2_frame, text="200", width=3)
        t2_label.pack(side=tk.LEFT, padx=5)
        ...
        def update_preview(*args):
            try:
                ...
                t1_label.config(text=str(t1))
                t2_label.config(text=str(t2))
