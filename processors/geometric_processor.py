...
class GeometricProcessor(BaseProcessor):
    ...
    def resize_image(self, image):
        ...
        self.center_window(dialog, "400x300")
        ...
        # Note: percent_var is used in Entry, so it might be fine, but let's check if there's a label for the scale itself?
        # The scale is linked to percent_var, which is displayed in Entry. The Entry formats it? 
        # Actually, Entry shows raw value. But user can type in Entry.
        # Let's leave Entry as is, it's editable. But maybe format it when scale changes?
        # In update_by_percent:
        # percent_var.set(round(width_var.get() / w * 100, 1)) -> This rounds it.
    ...
    def flip_image(self, image):
        ...
        self.center_window(dialog, "400x400")
        dialog.resizable(False, False)
        dialog.grab_set()
    ...
    def move_image(self, image):
        ...
        self.center_window(dialog, "700x550")
        dialog.resizable(False, False)
        dialog.grab_set()
        ...
        tx_label = ttk.Label(controls_frame, text="50", width=4)
        tx_label.grid(row=0, column=2, padx=5)
        ...
        ty_label = ttk.Label(controls_frame, text="50", width=4)
        ty_label.grid(row=1, column=2, padx=5)
        ...
        def update_labels(*args):
            tx_label.config(text=str(int(tx.get())))
            ty_label.config(text=str(int(ty.get())))
        tx.trace("w", update_labels)
        ty.trace("w", update_labels)
        update_preview()
    ...
    def rotationMatrix2d(self, image):
        ...
        angle_label = ttk.Label(controls_frame, text="45.0", width=5)
        angle_label.grid(row=0, column=2, padx=5)
        ...
        scale_label = ttk.Label(controls_frame, text="1.00", width=5)
        scale_label.grid(row=1, column=2, padx=5)
        ...
        def update_labels(*args):
            angle_label.config(text=f"{angle.get():.1f}")
            scale_label.config(text=f"{scale.get():.2f}")
        angle.trace("w", update_labels)
        scale.trace("w", update_labels)
